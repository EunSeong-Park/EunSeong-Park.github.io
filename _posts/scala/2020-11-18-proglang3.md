...
### GC Safety
GC는 안전하게 동작하며, 또한 그래야만 한다. (물론 편리함도 있겠지만) 그것이 GC를 쓰는 이유이기 때문이다. 여기서 안전하다는 것은 **"GC 이전과 이후 expression의 evaluation이 동일함"**을 의미하는데, 이러한 측면에서 우리의 GC는 안전하게 동작한다. 왜? ($\rho, \sigma \vdash E \implies v, \sigma '$)에서, $\sigma$에서 사용된 $Loc$들은 $\text{reach}(\rho, \sigma)$에 포함되어 있음이 보장되기 때문이다. 증명은 알아서 하자.
